...
#### 4. 加密字符串
- 选择一个文件作为解密实现的写入位置（`String` 的 `decrypt` 扩展方法会写入该文件）
- 开启后，混淆过程中会使用 **AES-128-CBC** 对所有明文字符串进行加密，且每次都会随机生成加密密钥
- **注意**：运行时解密会带来额外性能开销，请根据业务场景按需开启

#### 5. 删除注释
- 开启后，混淆过程中会基于 **AST** 移除代码中的**所有类型注释**（**非正则匹配/替换**）
- 删除范围由语法树精准界定，**仅移除注释，不会误删任何业务或可执行代码**

#### 6. 混淆文件名和文件夹名
- 开启后，混淆过程中会同时混淆**文件名**与**文件夹名**
- 该功能的映射会写入**本次混淆记录**；后续基于同一条记录做增量混淆时，不会因路径再次被重命名而产生不必要的工程结构差异
- **注意**：若项目不支持 Xcode 的 **Folder-backed** 特性，混淆完成后需要将已混淆的文件**手动导入**到工程中
...
- **移除注释进度**：移除注释处理进度（如果开启）
- **加密字符串进度**：字符串加密处理进度（如果开启）
- **混淆日志**：实时输出混淆操作日志
...
### v1.1.9
- 优化部分 UI
- 新增多语言支持，目前支持英语、简体中文、繁体中文、韩语、日语
- 修复问题：彻底解决开启字符串混淆后导致 Multipart 网络请求崩溃、长文本转义字符失效的问题

### v1.1.8
- 优化 UI 界面

### v1.1.7
- 优化 `CodingKeys` case 的属性绑定
- 新增明文字符串加密功能
- 新增文件名及文件夹路径混淆功能
- 新增基于 AST 移除代码内所有注释（不限注释类型）功能

### v1.1.6 🔒 定制版本
- 新增明文字符串的拆分和混淆功能

> ⚠️ **定制版本 - 不公开下载**：此版本不公开下载，如有需要请联系
